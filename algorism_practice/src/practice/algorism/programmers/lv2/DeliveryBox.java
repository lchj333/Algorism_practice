...
public class DeliveryBox {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(db.solution(new int[] {4, 3, 1, 2, 5}));
        System.out.println(db.solution(new int[] {5, 4, 3, 2, 1}));
        System.out.println(db.solution(new int[] {1, 2, 4, 3, 5}));
        System.out.println(db.solution(new int[] {2, 1, 4, 3, 6, 5, 8, 7, 10, 9}));
    }

    public int solution(int[] orderArray) {
        int now = 0;
        int orderIdx = 0;
        List<Integer> truck = new ArrayList<>();
        Stack<Integer> subContainer = new Stack<>();

        while(orderArray.length > orderIdx && now <= orderArray[orderIdx]) {
            System.out.println("orderArray[" + orderIdx + "] : " + orderArray[orderIdx] + ", now : " + (now+1));

            while(++now == orderArray[orderIdx]) {
                // 바로 트럭 적재
                truck.add(now);
                orderIdx++;
                print(truck, subContainer);
            }

            while(subContainer.size() > 0 && subContainer.peek() == orderArray[orderIdx]) {
                // 서브 컨테이너 -> 트럭
            ...
            // 서브 컨테이너 적재
            subContainer.push(now);
        }

        return truck.size();
    }

    public void print(List<Integer> truck, Stack<Integer> subContainer) {
...
예를 들어, 영재가 5개의 상자를 실어야 하며, 택배 기사님이 알려준 순서가 기존의 컨테이너 벨트에 네 번째, 세 번째, 첫 번째, 두 번째, 다섯 번째 놓인 택배상자 순서인 경우, 영재는 우선 첫 번째, 두 번째, 세 번째 상자를 보조 컨테이너 벨트에 보관합니다.
그 후 네 번째 상자를 트럭에 싣고 보조 컨테이너 벨트에서 세 번째 상자 빼서 트럭에싣습니다. 다음으로 첫 번째 상자를 실어야 하지만 보조 컨테이너 벨트에서는 두 번째 상자를, 기존의 컨테이너 벨트에는 다섯 번째 상자를 꺼낼 수 있기 때문에 더이상의 상자는 실을 수 없습니다.
