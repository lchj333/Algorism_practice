package practice.algorism.programmers.lv3;

public class IntegerTriangle {
    public static void main(String[] args) {
        System.out.println(new IntegerTriangle().solution(new int[][] {{7}, {3, 8}, {8, 1, 0}, {2, 7, 4, 4}, {4, 5, 2, 6, 5}}));
    }

    public int solution(int[][] triangle) {
        for(int row = triangle.length-1; row > 0; row--) {
            for(int col = 0; col < triangle[row].length-1; col++) {
                // 같은 뿌리의 두 수 중 큰값을 뿌리에 더하여 점차 줄여나간다.
        ...
        for(int row = 0; row < triangle.length; row++) {
            for(int col = 0; col < triangle[row].length; col++) {
                System.out.printf("%d ", triangle[row][col]);
            }
            System.out.printf("\n");
        }
