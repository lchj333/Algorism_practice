...
public class ReportResult {
    ...
    public int[] solution(String[] id_list, String[] report, int k) {
        ...
        for(String repo : report) {
            String[] info = repo.split(" ");
            HashSet<String> tempSet;
            if(reportMap.get(info[1]) == null) {
                tempSet = new HashSet<>();
            }else {
                tempSet = reportMap.get(info[1]);
            }
            tempSet.add(info[0]);
            reportMap.put(info[1], tempSet);
        }

        HashMap<String, Integer> mailMap = new HashMap<>();
        for(String key : reportMap.keySet()) {
            HashSet<String> tempSet = reportMap.get(key);
            if (tempSet.size() >= k) {
                for(String user : tempSet) {
                    mailMap.put(user, mailMap.get(user) != null ? mailMap.get(user)+1 : 1);
                }
            }
        }

        for (int i = 0; i < id_list.length; i++) {
            Integer cnt = mailMap.get(id_list[i]);
            answer[i] = cnt != null ? cnt : 0;
        }
    ...
    public static void main(String[] args) {
        ...
        String s = "foobar";
        int[] answer = new ReportResult().solution(id_list, report, k);
        for(int an : answer) System.out.println(an + " ");
    }
}
/*
신고 결과 받기
